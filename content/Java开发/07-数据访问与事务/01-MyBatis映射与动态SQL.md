...
你已经知道想执行哪条 SQL，只是不想反复写参数绑定和结果集转换，MyBatis 就适合从这里理解。先看一个 Mapper 方法怎样对应 SQL，再学动态条件和缓存，始终保留查看实际语句的习惯。
...
**这里容易混淆的是：** Mapper 代理可作为线程安全的注入对象，但代理内部使用的会话必须由框架按操作/事务管理。
...
**这里容易混淆的是：** `${}` 不做参数化，直接接收请求值会形成注入入口；即使是 ORDER BY 也应映射到固定列名。
...
**这里容易混淆的是：** 客户端一次构造超长 IN 列表既可能超过数据库限制，也会增加解析与网络成本。
...
**这里容易混淆的是：** 二级缓存不是 Redis 的替代，也不保证与数据库外部写入实时一致。
...
先把这里的 SQL 当作 Mapper 最终需要表达的查询，再考虑参数绑定和结果映射。问号是 JDBC 占位符；写进 MyBatis 映射时应使用对应的参数绑定语法，并按目标数据库调整分页方言。
