...
调用 `save` 或修改实体字段时，数据库未必就在那一行立刻更新。理解实体状态、持久化上下文和 flush，才能读懂 JPA 程序。下面也会区分规范、Hibernate 实现和 Spring Data 仓库，避免把三者混叫成一个东西。
...
**这里容易混淆的是：** 看到对象字段已变不代表其他事务已可见；可见性由数据库隔离和提交决定。
...
**这里容易混淆的是：** 把所有关系设为 EAGER 会产生笛卡尔积、重复数据或不可控查询，并不等于消除 N+1。
...
**这里容易混淆的是：** Repository 方法名很长通常说明查询语义已超出派生查询的可读范围。
...
**这里容易混淆的是：** 自动重试整个非幂等业务可能重复外部副作用；重试边界必须明确。
