...
数据库访问框架省掉了不少重复代码，但连接、SQL 和事务依然存在。这一阶段沿着一次读写操作，比较 MyBatis 与 JPA 分别接管了什么。遇到性能或一致性问题时，仍要能回到实际 SQL 和事务上检查。
...
**可以这样检查自己是否理解：** 能识别列名、空值和一对多结果展开问题。

### 2. 持久化上下文

JPA 在一个上下文中保证同一实体标识对应同一受管理实例，并在 flush 时执行脏检查生成 SQL。

**可以这样检查自己是否理解：** 能区分 persist、merge、flush 与 commit。

### 3. N+1 查询

先查 N 个主体，再为每个主体额外查关联，导致请求数随数据量线性增长。

**可以这样检查自己是否理解：** 能用日志/指标发现并用 join fetch、实体图或批量查询修复。

### 4. 连接池与迁移

连接池复用有限数据库会话；迁移工具按版本执行可审计的 schema 变更。

**可以这样检查自己是否理解：** 能估算池大小并写前向兼容的两阶段变更。
