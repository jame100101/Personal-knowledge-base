...
订单创建后可能要预留库存、等待支付、发送通知，其中某一步失败时，其他步骤该怎么办？这个项目用具体状态变化练习事件与补偿。先写清允许的结果，再设计服务之间的消息。
...
**这里容易混淆的是：** 消息 broker 的持久化不覆盖数据库双写一致性。
...
**这里容易混淆的是：** 补偿是新业务动作，可能失败或不可逆，不能写成简单反向函数。
...
**这里容易混淆的是：** 只看 HTTP 延迟会遗漏异步业务完成的用户体验。
...
事务后监听器与定时扫描分别展示通知和补偿入口，方法体仍需实现。若要求进程崩溃后也能补发，待处理记录必须持久化；仅依赖内存事件不够。练习时故意在提交后中断进程，检查是否能补齐。
