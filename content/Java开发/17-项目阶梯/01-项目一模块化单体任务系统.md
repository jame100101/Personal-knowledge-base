...
先做一个任务管理系统：用户登录、创建任务、修改状态，再查询自己的记录。业务不复杂，正好用来把接口、数据库、权限和测试连起来。把这些基础做稳，再考虑事件和部署扩展。
...
**这里容易混淆的是：** 模块化单体不是按 controller/service/repository 横向分包，而是先按业务能力分模块。
...
**这里容易混淆的是：** 只完成 CRUD 不足以验证后端能力，必须包含并发、错误和权限路径。
...
**这里容易混淆的是：** 覆盖率达到阈值仍可能遗漏关键不变量，应以风险清单验收。
...
**这里容易混淆的是：** 能启动不等于可运营；必须演示一次错误定位和恢复。
...
这里用订单命令演示 Command 与 Handler 的关系，结构同样可以用于任务创建。多个 public 类型应分别放入对应文件，并补齐 UUID 等 import。它只规定接口；重复请求和业务校验仍要在实现中处理。
