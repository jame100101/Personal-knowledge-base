...
远程调用写起来像普通方法，并不表示它具有相同的失败行为。下面从请求编码、连接、超时和返回结果看 RPC，再比较 HTTP、gRPC、Dubbo 等工具各适合什么需求。
...
**这里容易混淆的是：** HTTP 200 包裹所有错误会破坏缓存、网关、监控和客户端通用处理。
...
**这里容易混淆的是：** 在网关、客户端库和服务网格同时重试会放大流量，重试所有权需唯一且可观测。
...
**这里容易混淆的是：** 二进制协议通常更紧凑，但端到端性能还受业务处理、TLS、连接复用和消息大小影响。
...
**这里容易混淆的是：** 增加工作线程不会自动解决下游阻塞；队列无界只会把过载变成内存和尾延迟问题。
...
这是一份服务协议定义，不是服务实现。`request_id` 为幂等处理提供标识，但是否去重仍由服务端实现。生成客户端和服务端代码后，还要安排超时、错误映射和认证，再测试重复请求。
