...
两台机器之间的网络断开时，系统仍要决定是否接受新的读写请求。CAP 讨论的就是带有明确前提的取舍，不是随意选两个字母。下面会把术语放回具体故障场景，再介绍复制与共识。
...
**这里容易混淆的是：** timeout 不是失败证明；无条件重试可能重复扣款或制造重试风暴。
...
**这里容易混淆的是：** 把数据库贴成永久的 CP/AP 标签会掩盖具体配置、操作类型和故障条件。
...
**这里容易混淆的是：** 最终一致只说明没有新更新后副本趋同，并未给出收敛时间或冲突语义。
...
**这里容易混淆的是：** Raft 不是分布式锁 API；上层仍需 fencing token、租约和业务状态机。
...
版本比较展示“只接受基于当前版本的修改”这一思路，不是完整并发控制实现。在真实数据库中，检查版本和写入需要由同一原子操作完成，否则检查之后仍可能被其他请求修改。
