...
下游已经很慢时，继续重试可能把它压得更严重。限流、熔断和重试的目的不同，组合方式也很重要。先看一次请求经过哪些地方，再给每个机制设定范围、等待上限和恢复条件。
...
**这里容易混淆的是：** 把业务编排全部塞入网关会形成难测试、难扩展的新单体。
...
**这里容易混淆的是：** 会话黏性隐藏状态问题且削弱均衡；确有需求时仍需故障迁移方案。
...
**这里容易混淆的是：** 排队不创造容量；无界排队会把拒绝转成超时、内存增长和过期工作。
...
**这里容易混淆的是：** 熔断器按依赖和操作隔离；全局一个熔断器会让无关功能互相拖累。
...
依赖 Resilience4j Retry，导入 `java.util.concurrent.Callable`、`java.io.IOException`、`java.time.Duration` 和 Resilience4j 类型。外围方法需处理或声明 `Exception`；`inventory.reserve()` 是业务接口，约定可抛 `IOException`。`Supplier.get()` 不能直接声明该受检异常，因此这里使用 `Callable.call()`。

这里展示重试次数与等待设置，假设调用接口能以匹配的异常形式报告失败。若返回的是正常值包装的错误，就不会自动命中异常规则。对库存预留这样的写操作，先实现幂等，再考虑重试。
...
Callable<Reservation> call = Retry.decorateCallable(retry, inventory::reserve);
return call.call(); // 只重试具备幂等键的操作
```
