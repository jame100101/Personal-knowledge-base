...
服务实例不断增加或重启，调用方怎样知道该连哪个地址？配置改变后，又怎样让正确的实例收到更新？注册发现与配置管理分别回答这些问题，下面再看 Nacos 如何参与其中。
...
**这里容易混淆的是：** 注册成功只表示控制面记录存在，不证明业务端点已经 ready。
...
- 本地缓存可在短暂失联时提供已知实例；前提是缓存有效且实例仍可达，不能保证缓存中的实例仍健康。
- 故障实例剔除速度与误判风险平衡。

**这里容易混淆的是：** 健康检查频率越高并不总更可靠；网络抖动可能造成频繁摘挂和负载震荡。
...
**这里容易混淆的是：** 动态刷新不是事务；多个实例看到新配置的时间可能不同，业务逻辑应容忍短暂混合版本。
...
**这里容易混淆的是：** 把注册中心做成所有请求的同步依赖会使控制面故障直接扩大为数据面故障。
...
配置片段用环境变量传入地址与命名空间。它没有展示完整配置导入和认证方式，需要按所选 Spring Cloud Alibaba 与 Nacos 版本补齐。启动后检查实例是否进入预期环境，而不只确认应用没有报错。
