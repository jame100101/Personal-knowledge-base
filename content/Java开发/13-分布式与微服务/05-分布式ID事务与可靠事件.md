...
订单、库存、支付分别由不同服务管理后，原来一个数据库事务能完成的工作被拆开了。下面从“哪些结果绝对不能出现”开始，选择幂等、事件或补偿机制，而不是先寻找万能的分布式事务注解。
...
**这里容易混淆的是：** 按时间大致有序的 ID 不是严格业务顺序，也不应作为全局时钟。
...
**这里容易混淆的是：** 2PC 保证的范围取决于参与资源和实现，并不包含未参与的外部 HTTP 副作用。
...
**这里容易混淆的是：** Outbox 解决数据库与事件记录的原子性，不保证 broker 和所有消费者恰好一次。
...
**这里容易混淆的是：** 某些副作用不可逆；设计时需设置确认点、人工流程或风险准备金。
...
下面是单个数据库事务的示意，不是跨服务事务方案。它帮助对照拆服务前后的变化：两个账户若不再由同一事务管理，就需要重新设计一致性、幂等或补偿，不能继续依赖这一段代码的保证。
