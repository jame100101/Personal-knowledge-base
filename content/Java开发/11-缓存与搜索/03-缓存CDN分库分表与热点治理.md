...
访问变慢时，先检查查询和索引，往往比立即分库分表更划算。下面按从简单到复杂的顺序讨论缓存、CDN 和分片，并解释每多保存一份数据，会增加哪些同步和排错工作。
...
**这里容易混淆的是：** 删除缓存失败仍会陈旧；关键数据需要重试、事件或版本校验，不能声称强一致。
...
**这里容易混淆的是：** CDN purge 有传播延迟，敏感撤回需要短 TTL、版本 URL 或边缘鉴权配合。
...
**这里容易混淆的是：** 读副本不是无成本容量，长查询仍会占 IO/缓存并影响复制。
...
**这里容易混淆的是：** 分库分表不会自动提升单条复杂 SQL，且会把 JOIN、事务、迁移和查询复杂度移到应用/中间件。
...
带内容指纹的文件名适合长时间缓存：内容变了，就生成新文件名。示例中的响应头只适合这类资源，不应原样套到会变化的 HTML 或包含个人数据的响应上。也要确认发布时旧资源仍能被访问。
