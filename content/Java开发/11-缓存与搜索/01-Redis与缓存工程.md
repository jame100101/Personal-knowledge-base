...
把查询结果放进 Redis 后，下一次读取可能更快，但数据修改后这份缓存怎么办？从这个问题出发，再学过期、淘汰、持久化与复制，能把各种功能连起来，而不是分别背命令。

Redis 不只是缓存；正确使用要理解数据类型、过期/淘汰、RDB/AOF、复制、Sentinel、Cluster 和缓存一致性。

## 先明确缓存的身份：它是不是原始数据

假设商品名称存在数据库，Redis 中放的是查询结果的副本。读请求先看 Redis，没找到再查数据库并回填，这叫 cache-aside。它的用途是减少重复查询，不是让 Redis 和数据库自动形成一个事务。

更新商品后，需要处理旧副本。即使先更新数据库再删除缓存，也可能碰到一个较慢的旧查询晚到并重新写入旧值。因此要按业务接受的陈旧时间选择过期、版本校验、失效通知等策略。延迟双删可以降低某些竞态出现的机会，但不是对所有时序成立的强一致性证明。

同样，Redis 锁往往带租期。任务停顿太久、租期已到而另一个执行者取得锁时，旧执行者可能仍继续写入。先理解“有超时的占用凭证”，再讨论续约、唯一持有者标识及存储侧约束。重要业务不能只凭一个锁 key 存在就断言排他性始终成立，设计时对照 [Redis 锁的官方讨论](https://redis.io/docs/latest/develop/clients/patterns/distributed-locks/)。
...
**这里容易混淆的是：** 单条命令原子不等于由多条命令组成的业务流程原子。
...
**这里容易混淆的是：** 设置 TTL 不保证到点立即释放内存；业务不得依赖物理删除时间。
...
**这里容易混淆的是：** Redis 复制默认异步，高可用不等同于零数据丢失。
...
**这里容易混淆的是：** 任何“先库后缓存/先缓存后库”的简单顺序都存在并发窗口，需按一致性要求量化。
...
**这里容易混淆的是：** 锁客户端认为“仍持有”并不证明租约未过期；最终资源必须拒绝旧 fencing token。
...
按顺序读这段 cache-aside 代码：先查缓存，未命中再读数据库，最后写回。TTL 加入少量随机量用于分散过期时刻。缓存不可用、并发回填以及数据更新后的失效策略，需要结合前面的讨论另行实现。
