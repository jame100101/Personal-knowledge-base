...
优化之前，先把“慢”说具体：哪条操作，在多大负载下，等待了多久？然后保留相同条件，一次改一个因素。下面把容量测试和 JVM 诊断连起来，避免把偶然波动当成优化效果。
...
**这里容易混淆的是：** Little 定律要求稳定长期平均；系统积压持续增长时直接套用会误导。
...
**这里容易混淆的是：** 一次 `System.nanoTime` 循环不是可靠 JVM 微基准。
...
**这里容易混淆的是：** 对象数量多不等于泄漏；从 GC Roots 的意外保留路径才是关键证据。
...
**这里容易混淆的是：** 增加堆可能减少 GC 频率，也可能增加内存成本和故障转储时间；需要测量。
