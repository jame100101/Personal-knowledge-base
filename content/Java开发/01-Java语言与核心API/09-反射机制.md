...
平常我们在源码里直接调用方法；框架却经常要等程序运行后，才知道要创建哪种对象、读取哪个字段。反射提供了这种检查能力。下面从 `Class` 开始看它怎样工作，也说明为什么它不该替代普通的方法调用。
...
`Class.forName(name)` 默认会初始化类。若类名来自外部输入，应该在加载前限定允许的类和加载器，必要时使用不初始化的重载；`asSubclass` 只校验类型关系，不是安全授权。类型兼容的插件照样可以执行有副作用的代码。
