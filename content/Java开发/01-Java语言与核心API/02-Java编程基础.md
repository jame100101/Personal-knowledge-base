...
变量保存什么，条件决定走哪条路，循环决定一段代码执行几次——这些规则构成了后面所有程序的基础。本章沿用课件顺序，但不要只看语法：每学一个运算符或循环，都试着手算结果，再运行核对。
...
窄类型参与算术运算通常先提升为 `int`；宽化转换不一定保留所有精度：`int` 到 `float`、`long` 到 `float/double` 都可能舍入；窄化转换需显式强制并可能截断高位。复合赋值隐含一次转换。
