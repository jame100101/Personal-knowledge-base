...
当两个类有相似行为时，是继承一个父类，还是把公共功能交给另一个对象？这一章不只介绍语法，也讨论这种选择。先看重写和多态怎样影响实际调用，再判断抽象类、接口和组合各适合什么场景。
...
`super.member` 用于访问父类中可访问的成员，例如调用被重写的父类方法。构造器规则要按 Java 版本理解：“显式 `super(...)` 或 `this(...)` 必须放在第一条语句”是旧教程的规则，Java 25 已正式支持灵活构造器体。

现在可以先检查参数、计算局部变量，再调用父类构造器。好处是无效参数可以在进入父类构造逻辑前被拒绝。不过这段前置代码处于“早期构造上下文”，不是一个已经可自由使用的对象：实例方法调用、实例字段读取等仍受限制；允许的本类字段赋值也要遵循具体规则。日常代码先用参数校验和局部计算这两种简单用途，不要把任意业务逻辑搬到 `super(...)` 前面。

例如，`Child(int n)` 可先执行 `if (n < 0) throw new IllegalArgumentException();`，再执行 `super(n);`。父类构造器返回后，才继续子类的实例初始化器及构造器后续部分；不要再概括成“所有子类代码都在父类构造后运行”。版本和限制见 [Java 25 灵活构造器体](https://docs.oracle.com/en/java/javase/25/language/flexible-constructor-bodies.html)。
...
## 3.9 完整可运行练习：接口、多态与组合

保存为 `PolymorphismDemo.java`，执行 `java PolymorphismDemo.java`（JDK 21+）。下面只模拟展示价格，不连接真实支付服务。

```java
public class PolymorphismDemo {
  interface PriceRule { long price(long cents); }
  static final class Regular implements PriceRule {
    public long price(long cents) { return cents; }
  }
  static final class TenOff implements PriceRule {
    public long price(long cents) { return Math.max(0, cents - 1000); }
  }
  static final class Checkout {
    private final PriceRule rule;
    Checkout(PriceRule rule) { this.rule = java.util.Objects.requireNonNull(rule); }
    long quote(long cents) {
      if (cents < 0) throw new IllegalArgumentException("金额不能为负");
      return rule.price(cents);
    }
  }
  public static void main(String[] args) {
    System.out.println(new Checkout(new Regular()).quote(2500)); // 2500
    System.out.println(new Checkout(new TenOff()).quote(2500));  // 1500
  }
}
```

`Checkout` 保存的字段类型始终是 `PriceRule`，调用的却是实际对象实现的方法，这就是此处的动态分派。结算类通过持有规则对象实现组合，不必继承每一种价格规则，也不需要按具体类型写分支。

试着添加“满 5000 分减 1000 分”的实现，再测试 4999、5000、5001 三个边界。注意契约：上例约定 `Checkout` 会检查输入，规则实现必须对非负输入返回非负报价。接口写出方法形状，文字契约和测试补充编译器无法证明的业务含义。
