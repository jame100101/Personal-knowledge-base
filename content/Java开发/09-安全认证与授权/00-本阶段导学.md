...
登录成功只证明了身份，不代表用户可以读取任何订单或修改任何账户。这一阶段从认证与授权的区别开始，再看会话、输入、依赖和日志中的风险。先把业务允许的操作写清，配置才有依据。
