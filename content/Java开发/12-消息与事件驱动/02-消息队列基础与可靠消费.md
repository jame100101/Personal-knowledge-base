...
下单接口把“发送通知”交给后台处理，可以缩短用户等待，但也增加了一个问题：后台处理到一半崩溃怎么办？本章围绕投递、确认和重复处理讲消息可靠性，先把一条消息的生命周期弄清楚。
...
**这里容易混淆的是：** 所谓“发送成功”必须说明成功到哪一层、在何种副本/落盘配置下。
...
**这里容易混淆的是：** 框架的 exactly-once 通常限定于特定 broker 读写事务，不自动涵盖任意数据库和 HTTP 服务。
...
**这里容易混淆的是：** 全局顺序显著限制并行度；业务多半只需要单实体顺序。
...
**这里容易混淆的是：** 只增加消费者数受分区数、下游容量和 key 热点限制。
...
先处理业务再记录事件，必须处于同一数据库事务。`existsById` 只是快速判断，两个并发请求仍可能同时通过；去重表需要唯一约束，冲突时应让整个事务回滚并按消费策略处理。外部 HTTP 副作用不在这个本地事务内。
