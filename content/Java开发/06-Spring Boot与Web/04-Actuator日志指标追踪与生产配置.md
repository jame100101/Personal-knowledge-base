...
应用已经上线，却有人反馈“有时很慢”，此时需要的不再是启动日志。指标帮助看趋势，追踪帮助找慢在哪一段，日志补充具体原因。下面介绍 Actuator 怎样提供入口，以及哪些信息不宜直接公开。
...
**这里容易混淆的是：** 数据库短暂不可用通常不应让 liveness 失败并触发所有实例重启。
...
**这里容易混淆的是：** 日志量不是可观测性质量；无结构、无关联和无语义的海量日志仍难诊断。
...
**这里容易混淆的是：** 客户端计算平均百分位不能在实例间正确聚合；使用可合并直方图桶。
...
**这里容易混淆的是：** trace 显示相关调用链，不自动证明因果；仍需日志、指标和代码证据交叉验证。
...
配置展示了端点暴露和追踪采样，不包含依赖与访问控制。`prometheus` 端点还需要相应 registry 依赖。先在测试环境观察哪些路径可访问，再决定生产中由谁访问；采样比例也不代表会保存全部请求。
