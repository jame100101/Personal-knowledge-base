...
先做一个能返回 JSON 的接口，再追问：请求是怎样进入这个方法的，参数由谁转换，异常又由谁变成响应？这一阶段沿着这条路径学习 Spring Boot 与 Web，不把启动成功当作已经理解全部流程。
