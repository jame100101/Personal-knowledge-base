...
接口好不好用，不只取决于成功时返回什么。重复提交会怎样，参数错误怎样说明，旧客户端还能不能继续调用，同样需要约定。下面用资源、状态码和错误响应把这些行为表达清楚。
...
**这里容易混淆的是：** 幂等表示重复相同请求的预期服务端效果一致，不表示每次响应字节完全相同。
...
**这里容易混淆的是：** 幂等键不是普通缓存 key；必须定义作用域、过期、请求内容冲突和处理中状态。
...
**这里容易混淆的是：** 在响应增加字段对严格反序列化客户端也可能是破坏性变更，需要真实消费者契约验证。
