...
上传文件、发送邮件和保持 WebSocket 连接，都比普通 JSON 查询多出一些生命周期问题。先想清数据有多大、任务会持续多久、失败后能否重试，再看各自的实现。OpenAPI 则帮助双方把接口约定写明白。
...
上传成功、通知已排队和用户已收到邮件，为什么是三个不同的状态？接口应怎样表达？
