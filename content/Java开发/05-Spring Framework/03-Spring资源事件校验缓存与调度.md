...
除了创建对象，应用还会遇到读取资源、发布事件、校验参数和缓存结果这些重复工作。Spring 为它们提供了公共接口，但每个接口都有适用条件。本章按具体用途介绍，避免把一个注解理解成完整解决方案。
...
**这里容易混淆的是：** Resource.getFile 只适用于真实文件资源；jar 内资源应使用 InputStream。
...
**这里容易混淆的是：** 标记 @Async 后，发布返回不表示监听成功；进程崩溃可能丢事件。
...
**这里容易混淆的是：** 前端校验只改善体验，后端与数据库仍必须执行权威校验。
...
**这里容易混淆的是：** @Scheduled 默认并不保证集群只有一个节点执行，也不自动补跑错过任务。
...
这里展示两个不同入口：事务提交后处理事件，以及按时间触发扫描。它们不是可靠消息系统的替代品；如果提交后进程崩溃也必须保证通知送达，还要设计持久化记录和补发机制。
