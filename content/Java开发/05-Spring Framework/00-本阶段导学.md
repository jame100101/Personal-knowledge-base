...
当服务需要仓库对象，仓库又需要数据库连接时，谁来创建并连接它们？Spring 容器接手了这类工作。先理解对象如何装配，再学习代理和事务，你就能解释注解背后的行为，而不只是记住写法。
...
**可以这样检查自己是否理解：** 能用纯 Java 配置装配两个组件并写替身测试。

### 2. Bean 生命周期

Bean 从定义、实例化、依赖注入、初始化回调到销毁回调经历多个阶段；作用域决定实例共享边界。

**可以这样检查自己是否理解：** 能解释 singleton Bean 为何不应保存请求可变状态。

### 3. 代理与 AOP

Spring 常用代理包裹目标对象，在调用前后执行横切逻辑。只有经过代理的调用才会被拦截。

**可以这样检查自己是否理解：** 能复现同类自调用导致事务注解未生效。

### 4. 声明式事务

事务拦截器依据传播、隔离、只读和回滚规则控制底层资源事务。数据库事务不会自动覆盖远程调用。

**可以这样检查自己是否理解：** 能划分短事务边界并说明异常回滚规则。
