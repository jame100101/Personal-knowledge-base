...
会写后端接口之后，可以亲手做一个能提交表单、显示列表的小页面。这样就能理解浏览器怎样组织内容、发送请求和保存界面状态。先用 HTML、CSS 和 JavaScript 搞清基本行为，再引入 Vue 与 TypeScript。
