...
引入 Vue 和 TypeScript 之后，组件更好组织了，但接口数据仍可能格式错误，页面也仍可能卡顿。本章把类型检查、运行时校验、测试和构建放在各自的位置，再看怎样验证用户实际看到的页面。
...
类型检查通过后，接口仍返回错误数据，你准备在哪里检查？怎样写测试覆盖这个场景？
