...
前端发出请求、后端返回成功，用户却没拿到文件或没有保持登录，问题常在两端约定或代理配置上。下面沿着认证、上传下载和实时连接分别检查，让整条用户操作都能完成。
...
**这里容易混淆的是：** 只共享 Java DTO 源码会把内部模型和外部契约耦合，且不能替代 HTTP 语义。
...
**这里容易混淆的是：** 把长期 token 放 localStorage 会扩大 XSS 窃取后果，应按架构评估更安全的 BFF/cookie。
...
**这里容易混淆的是：** 把大文件完整读入 JVM 堆会造成内存峰值，使用流式、配额和背压。
...
**这里容易混淆的是：** WebSocket 建连成功后连接可能随时中断，可靠业务仍需持久状态和重同步。
...
这里分别代理普通接口与 WebSocket 升级请求，`backend` 需要在 Nginx 配置中有对应定义。部署时还要确认超时、TLS 与后端对代理头的信任设置，不能只凭浏览器能连上就认为整条链路正确。
