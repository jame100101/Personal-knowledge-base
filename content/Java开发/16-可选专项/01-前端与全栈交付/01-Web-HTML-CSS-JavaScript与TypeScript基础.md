...
一个按钮从显示到点击生效，涉及内容结构、样式和事件三个层面。下面用这些具体行为理解 HTML、CSS 与 JavaScript，再说明 TypeScript 怎样帮助检查代码。它们协作完成页面，不是彼此的替代品。
...
**这里容易混淆的是：** CORS 由浏览器执行，服务器间调用不受其保护；后端仍需认证授权。
...
**这里容易混淆的是：** 给 div 添加 click 不会自动获得按钮的键盘、焦点和语义行为。
...
**这里容易混淆的是：** 响应式不是按设备品牌写断点，而是按内容在可用空间中的布局需求。
...
**这里容易混淆的是：** TypeScript 类型在运行时被擦除，`as` 断言不会转换或验证对象。
...
把例子保存成 HTML 页面，输入文字并提交，观察控制台结果。浏览器会先做原生表单校验，脚本再读取 FormData。这里阻止了默认提交，并没有向后端发送搜索请求；可以把网络请求作为下一步练习。
