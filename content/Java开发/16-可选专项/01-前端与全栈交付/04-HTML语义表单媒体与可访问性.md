...
一个看起来像按钮的 `div`，不一定能像真正的按钮那样用键盘操作。HTML 元素不仅决定外观起点，也提供语义和默认交互。先选合适的元素，再加样式和脚本，往往能少写很多补救代码。
...
**这里容易混淆的是：** `section` 通常需要可识别标题；用于布局的无语义容器可使用 `div`，但不要用 `div` 模拟原生按钮。
...
只用键盘能否填写并提交示例表单？每个输入控件的标签和错误信息是否能被理解？
