...
自动化脚本今天成功、明天随机失败，通常比没有脚本更让人困惑。下面围绕稳定的数据准备、元素定位和失败记录组织测试，让一次失败有信息可查，也让多条测试能互不干扰地运行。
...
**这里容易混淆的是：** 只对整段 JSON 快照容易产生噪声并遗漏语义；关键字段用明确断言。
...
**这里容易混淆的是：** CSS/XPath 长路径紧耦合实现，页面小改动会造成大量无业务意义失败。
...
**这里容易混淆的是：** 共享不可重置环境会产生顺序依赖和假阳性，优先建设可创建的环境。
...
**这里容易混淆的是：** 把 flaky 测试永久 quarantine 会逐步失去保护；隔离只是有期限的修复阶段。
...
测试从用户能看见的标签和按钮定位，再断言操作结果。示例假设已配置 `baseURL`、登录状态和测试客户数据。真正回归时，还应验证数据确实保存，而不是只出现了一条成功提示。
