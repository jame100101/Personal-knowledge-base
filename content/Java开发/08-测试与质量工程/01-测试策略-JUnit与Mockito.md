...
一个测试失败后，最好能直接告诉你哪条规则被破坏，而不是只显示两个很长的对象不相等。下面从业务行为出发写断言，再讨论什么时候使用 Mockito。替身用来隔离依赖，不是把所有真实逻辑都绕开。
...
**这里容易混淆的是：** “测试金字塔”是反馈和成本原则，不要求固定比例，也不排斥高价值集成测试。
...
**这里容易混淆的是：** 默认测试实例生命周期和并行配置会影响共享可变状态；不要依赖隐式顺序。
...
**这里容易混淆的是：** 大量 mock 链通常说明设计边界不清，且测试可能与实现耦合。
...
**这里容易混淆的是：** 单次并发测试通过不能证明没有竞态；需要状态空间、压力和 JVM 内存模型证据。
...
这个测试不仅检查抛出了异常，还检查余额没有变化。两条断言共同描述“拒绝扣款”的业务含义。需要在练习项目中提供 Account 与异常类；可以先故意写成先扣款再报错，观察测试能否发现问题。
