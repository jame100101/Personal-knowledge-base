...
设计用户表时，哪些字段必须有值，用户名能否重复，一次转账怎样避免只完成一半？这些问题会自然引出约束和事务。下面先用关系模型把数据说清，再用 SQL 表达查询与修改。
...
**这里容易混淆的是：** 外键不是天然性能灾难；是否使用取决于一致性要求、写入模式、分片边界与运维能力。
...
**这里容易混淆的是：** `= NULL` 不返回真，应使用 `IS NULL`；`NOT IN` 遇到 NULL 也可能产生 UNKNOWN。
...
**这里容易混淆的是：** 同名隔离级别在不同数据库的实现细节并非完全相同，结论以目标数据库文档和实验为准。
...
**这里容易混淆的是：** “使用索引”不等于更快；低选择性、回表、随机 I/O 或错误估算可能让全表扫描更合适。
...
**这里容易混淆的是：** 代码回滚并不会自动回滚数据；不可逆迁移必须有前滚修复和数据恢复方案。
...
这个查询按创建时间和 ID 一起翻页，ID 用来区分相同时间的记录。问号表示需要由驱动绑定的值，不是直接粘贴到控制台就能执行的文本。示例采用标准分页写法；换数据库时要核对其方言。
