...
一个 Java 服务启动后，就是操作系统中的进程：它打开文件、监听端口，也会受到权限和内存限制。下面先学会找到这个进程并读懂它的输出，再引入 Docker；容器没有让这些基本规则消失。
...
**这里容易混淆的是：** chmod 777 不是权限问题的通用处理；要检查所有者、组、目录执行位、ACL 和挂载选项。
...
**这里容易混淆的是：** 把未经验证文本拼入 `sh -c` 会造成命令注入；参数数组和固定命令结构更可靠。
...
**这里容易混淆的是：** 容器不是虚拟机；其进程共享宿主内核，资源和安全隔离取决于命名空间、cgroup 与配置。
...
**这里容易混淆的是：** 只把 `-Xmx` 设置等于容器内存上限会忽略非堆内存，容易触发容器 OOMKill。
...
这是一段 Bash 启动脚本，不是 PowerShell 命令。它先检查 jar 是否存在，再用 `exec` 让 Java 替代当前 shell，便于传递停止信号。内存百分比只是示例值，还要给线程栈和其他本地内存留余量。
