...
分支并不是把整个项目复制一份，而是指向某个提交的引用。理解这一点，再看合并、变基和回滚会清楚很多。下面从你每天会做的改动开始，逐步解释 Git 怎样保存历史，以及多人协作时如何避免误操作。
...
**这里容易混淆的是：** `reset` 移动引用并可能改 index/工作区；`revert` 创建反向提交，更适合共享历史。
...
**这里容易混淆的是：** rebase 后内容可能相同但提交身份不同；已被他人基于的历史需谨慎重写。
...
**这里容易混淆的是：** 分支模型本身不产生质量；自动化测试、评审和可回滚发布才构成控制闭环。
...
**这里容易混淆的是：** 删除或移动发布 tag 会破坏可追溯性；正式 tag 应视为不可变。
...
下面演示在功能分支提交并更新基线。执行前先确认 `src`、`test` 是当前项目的目录，并检查工作区。`rebase` 会重写该分支的提交；已经共享给别人的分支，需要先按团队约定协调。
