...
打出 jar 包只是交付的一步。接下来还要让它带着正确配置启动，判断是否健康，出现问题时找到原因并回退。这一阶段沿着上线过程介绍容器、部署和可观测性，而不是从一大段 YAML 开始。
...
**可以这样检查自己是否理解：** 能从运行实例追溯到提交、构建和镜像摘要。

### 2. 声明式编排

Kubernetes 控制器持续把实际状态收敛到期望状态；Pod 是可替换实例，不应保存唯一持久数据。

**可以这样检查自己是否理解：** 能解释 Deployment 滚动更新和 Service 选择器。

### 3. 探针与优雅停机

readiness 决定是否接流量，liveness 判断是否需重启；终止时先停止接流量再完成在途请求。

**可以这样检查自己是否理解：** 能验证 SIGTERM、宽限期和连接排空。

### 4. 可观测性与 SLO

日志、指标和 trace 是回答未知问题的遥测；SLO 用用户可感知指标定义可靠性目标和错误预算。

**可以这样检查自己是否理解：** 能从告警跳到仪表盘、trace、日志与 runbook。
