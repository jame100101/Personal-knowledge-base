...
从 Java 转到 TypeScript，最需要调整的是对“类型相同”的判断。两个对象即使没有声明实现同一个接口，也可能满足相同的结构要求。本章用熟悉的类、接口和泛型作比较，同时保留 JavaScript 运行时与 JVM 的差别。

类、接口和泛型是两边都熟悉的入口，但理解它们时要继续问“运行后还保留什么”。TypeScript 主要用结构描述 JavaScript 值，Java 的类和接口身份则与 JVM 的运行方式联系更紧密。后面的例子会反复用到这个区别。
