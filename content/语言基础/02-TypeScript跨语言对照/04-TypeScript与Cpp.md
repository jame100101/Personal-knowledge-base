...
C++ 模板和 TypeScript 泛型都使用尖括号，但它们对最终程序的影响很不同。前者会参与代码生成，后者通常只留下类型检查结果。把这个区别放在心里，再比较对象、资源释放和内存控制，很多表面相似的语法就能分清了。

阅读对照代码时，除了看输出相不相同，还要看它付出的成本在哪里：TypeScript 类型通常在检查后被擦除；C++ 的类型和模板会参与对象布局与生成代码。资源释放也因此需要用不同的方式理解。
