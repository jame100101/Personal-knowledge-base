...
这一章适合在学完基础后回查：某个能力是 TypeScript 自己提供的，还是来自 JavaScript、浏览器、Node 或第三方库？先把来源认清，再讨论它擅长什么、缺少什么，就不会把“语言没有内置”误解成“整个生态都做不到”。

下面把语言本身与外部能力分开列出。例如，类型系统可以约束代码，却不会因此提供某个操作系统接口。需要浏览器、Node、库或构建工具完成的事情，会注明来源；这样比较能力时才不会把层次混在一起。
