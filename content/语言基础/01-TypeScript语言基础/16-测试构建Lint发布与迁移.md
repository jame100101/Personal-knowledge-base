...
编辑器里没有红线，只能说明通过了当前配置下的类型检查。程序是否按预期工作、打包后是否能被别人使用，还需要另外验证。本章按开发到发布的顺序，把类型检查、测试、Lint 和构建各自负责的事情串起来。

可以把发布前的检查理解为几道不同的问题：类型是否一致，功能是否正确，代码是否违反约定，产物是否完整，别人能否导入使用？这些问题分别需要类型检查、测试、Lint、构建和使用验证来回答。
