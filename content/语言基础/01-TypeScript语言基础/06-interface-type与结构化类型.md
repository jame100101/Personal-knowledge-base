...
刚接触 TypeScript 时，同一个对象既能用 `interface` 描述，又能用 `type` 描述，确实容易困惑。先看它们共同解决的问题：描述对象需要有哪些成员。随后再比较扩展、合并和联合类型，按实际需要选择，而不必先站队。

普通对象结构通常两种都能描述，所以初学时不必因为选了其中一种而担心写错。真正需要比较的是后面的能力：是否要声明合并，是否要表达联合或类型运算，以及这份类型准备怎样被别人扩展。
...
同名接口会合并。这对库的模块增强很有用，对应用领域模型却可能让定义被意外扩展。希望避免同名接口声明合并时可使用 `type`；这不让对象成为禁止额外属性的 exact/sealed 类型；明确希望消费者扩展的公共对象契约可用 `interface`。
