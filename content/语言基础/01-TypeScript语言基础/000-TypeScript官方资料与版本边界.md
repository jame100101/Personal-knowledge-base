...
遇到不熟悉的语法，可以先在 Handbook 找解释；遇到配置报错，再查 TSConfig Reference。不必从头读完所有官方文档。本页把这些入口按用途整理在一起，并单独记录版本相关说明，方便你判断一个例子是否适用于手头的编译器。
...
2026-09-25 重新核对官方公告：TypeScript 7.0 已于 2026-07-08 发布，核心编译器和语言服务从 JavaScript/TypeScript 代码库迁移到 Go 原生实现，主要目标是显著改善检查和编辑器性能。它尽量保持 TypeScript 6.0 的类型检查与命令行行为兼容，但仍有三个工程边界：
