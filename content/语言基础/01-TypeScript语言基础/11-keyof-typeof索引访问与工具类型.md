...
假设用户对象已经有一份类型定义，表单、更新接口和字段选择器还要各抄一份吗？`keyof`、索引访问和工具类型能从已有定义推导出新类型。先理解推导前后的关系，再记各个工具的名字，会比背一张语法表更容易。

这些操作符处理的是类型，不是实际对象。例如，得到一组属性名的类型，并不等于执行了 Object.keys。把类型推导和运行时代码分开看，才能知道什么时候只是减少重复声明，什么时候还需要真正的函数。
