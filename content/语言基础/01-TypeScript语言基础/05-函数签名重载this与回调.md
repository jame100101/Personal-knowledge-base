...
写函数时，调用者最关心两件事：我能传什么，拿回来什么？函数签名就是对这两件事的说明。先掌握普通参数和返回值，再看重载、回调和 `this`；它们是在普通签名不够清楚时补充表达能力，而不是每个函数都必须使用的技巧。
...
如果函数要交给别的模块使用，写出返回类型可以让约定更清楚：以后修改实现，返回了不合适的值，编译器就能提醒你。像上面的 map 回调这样简短的局部函数，则可以让上下文帮助推断，不必每一处都重复标注。
