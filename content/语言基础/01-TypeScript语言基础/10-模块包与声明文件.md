...
编辑器能跳转到一个包的类型定义，程序运行时却提示找不到模块，这并不矛盾。类型文件和实际执行的 JavaScript 是两条需要对齐的路径。下面沿着一次 `import`，分别看编译器、Node 和构建工具会去哪里找文件。

排查 import 时分两步：先确认 TypeScript 找到的类型定义，再确认运行时或构建器找得到实际 JavaScript。编辑器不再报错，只证明第一步可能解决了；还要运行构建产物，检查第二步。

## 从拆成两个文件开始理解模块

模块让一个文件明确说明自己提供什么、使用什么。`export` 暴露需要给其他文件使用的值或类型，`import` 声明依赖。拆文件的目的不是把代码切碎，而是让功能边界和依赖方向看得见。

这里有两个阶段：TypeScript 先判断导入是否符合类型和模块规则，运行时再真正加载 JavaScript 文件。检查成功不代表 Node 一定找得到输出文件，所以还要确认扩展名、包的 `type` 字段以及最终目录结构。

`.d.ts` 像一份类型接口说明，它描述某个实现有什么能力，但通常不包含那个实现。声明里写“这个函数返回 number”，不会让实际返回字符串的第三方代码自动变正确。学习时打开生成的 JavaScript 和声明文件对照一次，再按[官方模块说明](https://www.typescriptlang.org/docs/handbook/2/modules.html)区分类型导入与运行时导入。
