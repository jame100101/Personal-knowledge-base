...
当多个类型只有一处规律性的差别时，手工维护很容易漏改。映射类型和条件类型可以表达这种规律。下面先看简单的输入与结果，再逐步读 `infer` 和模板字面量；如果一种写法已经难以向同事解释，就值得考虑更直接的方案。

可以先手写两个具体类型，找出它们之间重复的规则，再把规则抽成类型运算。如果一个普通联合类型已经足够清楚，就没有必要换成复杂递归。高级写法的价值在于减少维护，而不是增加阅读难度。
