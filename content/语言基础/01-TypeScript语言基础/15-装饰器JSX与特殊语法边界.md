...
装饰器、JSX 和一些特殊语法常出现在框架教程里，初看很像语言自带的魔法。这里先分清：哪些由 JavaScript 执行，哪些需要编译转换，哪些还取决于框架或历史配置。理解这一点，再读生成的代码就不容易混淆。

复制例子前，先看它依赖的编译目标与配置。有些写法遵循 JavaScript 运行规则，有些由 TypeScript 或框架转换；旧项目还可能使用历史上的装饰器方案。名字相同不保证配置与行为完全相同。
