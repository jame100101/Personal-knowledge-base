...
假设本地改了某篇课程，但管理员也在线上编辑过。发布脚本若无条件覆盖，会悄悄丢失另一份工作。应先比较线上正文与已知基线，无法确认一致就停止该发布；真正写入时再带 updated_at 或版本条件，防止检查之后又发生修改。

这叫乐观并发控制：允许大家先编辑，到提交时确认自己依据的版本仍有效。它与 Git 冲突检查类似，但数据库内容和 Git 文件是两个独立位置，都需要各自验证。备份可以帮助恢复，不能替代提交前的冲突检查。

发布成功后，用公开读取权限逐篇比较正文，确认文档 ID、slug、目录和排序未意外改变，再从首页进入文章。旧链接仍可访问、页面正文确实更新、代码块与目录正常，才能说明读者实际收到修订。

数据库的正文更新和 Vercel 代码部署不一定原子完成。涉及新字段时先设计兼容顺序；纯内容修订则保留结构并分别验收。遇到失败，先记录已完成哪些写入，再按清单恢复或继续，不要用重新导入全部记录冒险掩盖状态。
