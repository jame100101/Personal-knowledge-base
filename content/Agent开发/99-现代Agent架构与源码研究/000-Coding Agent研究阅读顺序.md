...
如果是第一次读 Coding Agent 源码，先挑一个项目，追踪一次工具调用就够了。熟悉后，再比较上下文、权限和恢复机制。下面按阅读目的整理入口，各报告的结论都受所列源码快照与证据范围限制。

> `last_verified: 2026-08-24`

所有报告采用固定 commit，区分 **CONFIRMED 源码事实、官方行为、INFERRED 架构推断与 UNKNOWN 未验证项**。
