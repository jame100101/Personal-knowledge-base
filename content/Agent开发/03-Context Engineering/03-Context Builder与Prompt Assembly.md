...
把所有规则、聊天记录和文件直接拼起来，很快就会遇到太长、重复或互相冲突的问题。Context Builder 要做的是先选择，再组织。下面沿着一次模型请求，看每份材料为什么被放进去，以及放不下时怎么处理。
...
缓存用于降低部分重复计算的延迟和成本，不自动保证语义一致。前缀内容变化后，要按供应商的缓存匹配规则重新判断是否命中，并更新本地记录的内容指纹。并不是每个 API 都要求调用方显式修改一个 `cache key`；有的服务自动按前缀匹配，有的提供额外缓存控制字段。日志里保留模板版本和内容摘要，通常比假定所有服务采用同一缓存接口更可靠。
