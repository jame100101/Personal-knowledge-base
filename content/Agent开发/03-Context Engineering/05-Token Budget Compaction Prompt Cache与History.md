...
想象你让 Agent 调查一个错误。它读过十个日志文件，但下一次模型请求只带了其中三段。那么模型这一轮能利用的是这三段，以及请求里的其他消息；磁盘上还存着七个文件，并不代表模型正在阅读它们。**History 是保存的过程，Context 是本轮实际送入的材料。**

Compaction 可以理解成“给接班的人写交接记录”。一份有用的记录要说清任务还差什么、哪些假设被推翻、修改了哪些文件，以及原始证据在哪里。只写“已经检查了项目”几乎没有帮助。摘要生成后最好抽查几项事实，还要保留原记录供重新读取。

Prompt Cache 则像复用此前处理过的请求前缀。它减少重复输入的处理成本，但不替你保存业务事实，也不保证回答正确。具体缓存命中条件、保存时间和费用由服务商规定。先保持稳定内容的顺序，再测量命中率；不要为了命中缓存而保留已经失效的工具权限。
