...
可以把已经能理解和生成语言的模型看成起点。后训练是在这个起点上，用示范、偏好或任务反馈继续调整模型，使它更符合目标行为。它会涉及参数或适配器的更新；仅仅给请求加一段提示词，不属于同一件事。

例如，Agent 总把工单编号填进用户名字段。先检查工具说明是否含糊、schema 是否缺少约束、返回错误是否清楚。如果这些已经改好，模型在大量同类任务中仍反复犯错，才值得考虑收集高质量示范用于训练。否则训练可能只是在掩盖一个容易修复的接口问题。

SFT 像给出“这个输入下，合理输出是什么”的示范；RL 则让策略根据奖励信号调整。两者都不是把所有日志直接喂进去：失败轨迹、泄漏的测试答案和过期工具格式会污染学习目标。先分开训练与评估数据，并用实际工具环境验收。原书对应内容现位于[第 8 章](https://bojieli.github.io/ai-agent-book/book/chapter8/)。
