...
并非接上模型的程序都需要 Agent。如果任务步骤已经固定，普通程序可能更简单可靠；如果下一步要根据实际结果临时选择工具或调整计划，Agent 往往更有发挥空间。不过，带条件分支的普通 Workflow 也能根据结果决定下一步；区别不在“有没有反馈”，而在动作选择有多少交给模型、路径有多少已由程序预先规定。下面用具体任务判断，而不是按产品名称判断。
