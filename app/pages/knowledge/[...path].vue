...
const childFolders = computed(() =>
  folders.value.filter((item) => item.parent_id === folder.value?.id).sort(sortFolders),
)
    ...
    <main class="document-page">
      ...
      <header class="document-header">
        ...
        <div class="document-meta">
          ...
          <span
            ...
            {{ new Date(document.updated_at).toLocaleDateString(dateLocale) }}</span
          >
      ...
      <nav class="document-nav">
        <NuxtLink v-if="previous" :to="documentPublicPath(previous, folders)">
          <ArrowLeft :size="15" /><span
            ><small>{{ t('previous') }}</small
            ><strong>{{ previous.title }}</strong></span
          >
        </NuxtLink>
        <span v-else />
  ...
  <main v-else-if="folder" class="folder-page">
    ...
    <header>
      ...
      <div class="folder-title">
        <span><Folder :size="26" /></span>
        <h1>{{ folder.name }}</h1>
      </div>
      <p>{{ folder.description || t('organizing') }}</p>
      <div class="folder-stats">
        <span>{{ childFolders.length }} {{ t('subdirectories') }}</span
...
.reader-grid {
  ...
  margin: 0 auto;
  display: grid;
  grid-template-columns: minmax(0, var(--kb-content-width)) var(
...
@media (max-width: 1180px) {
  .reader-grid {
    grid-template-columns: minmax(0, var(--kb-content-width));
  }
  .reader-grid > aside {
    position: static;
    order: -1;
