...
try {
  ...
  if (apply) {
    ...
    const backup = `.tools/content-backups/backend-${Date.now()}.json`
    ...
    for (const spec of missingFolders) {
      ...
      const { data, error } = await db
        ...
        .insert({
          ...
          description: '从请求、数据与业务边界学到后端架构、可靠性和部署实践',
          icon: 'Folder',
  ...
  if (docMap.size === documents.length) {
    ...
    await fs.mkdir('.tools/backend', { recursive: true })
    await fs.writeFile(
      '.tools/backend/publication.json',
